//
//  UINavigationBar+JCNavigationBarHandle.m
//  DiSiYeMeiEr
//
...
- (void)setNavView:(JCNavView *)navView{
    objc_setAssociatedObject(self, &myNavView_key, navView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)jc_navBarTitleColor:(UIColor *)color font:(UIFont *)font {
...
- (void)jc_navBarAlpha:(CGFloat)alpha isOpaque:(BOOL)opaque {
    ...
    [[self valueForKey:@"backgroundView"] addSubview:self.navView];
}

- (void)jc_navBarBackGroundColor:(UIColor *)color image:(UIImage *)barImage isOpaque:(BOOL)opaque {
    [self clearSystemLayerIsOpaque:opaque];
    if (!self.navView) {
        //状态栏高度
        CGFloat statusHeight = [UIApplication sharedApplication].statusBarFrame.size.height;
    ...
    if (color) {
        self.navView.backColor = color;
    }
    if (barImage) {
        self.navView.backImage = barImage;
    }
    //通过kvc找到系统导航栏背景层，把自定义层添加到背景层
...
- (void)jc_navBarBottomLineHidden:(BOOL)hidden {
    ...
    //如果是自定义图层
    [self customNavBar];
    if (self.navView.hiddenBottomLine != hidden) {
        self.navView.hiddenBottomLine = hidden;
...
- (JCNavView *)customNavBar {
    if (!self.navView) {
        self.navView = [[JCNavView alloc]init];
    }
    return self.navView;
}
@end
